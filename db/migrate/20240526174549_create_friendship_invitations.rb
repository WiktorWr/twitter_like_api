...
class CreateFriendshipInvitations < ActiveRecord::Migration[7.1]
  def change
    ...
    create_table :friendship_invitations do |t|
      ...
      t.enum :status, enum_type: :invitation_status, null: false, default: "pending"

      t.timestamps
    end

    add_index :friendship_invitations, [:sender_id, :receiver_id], unique: true, where: "status = 'pending'"
